...
function setBreadcrumb(siteTitle, cat, title){
  $('#breadcrumb').innerHTML = `
    ...
    ${cat?` » <a href="${withBase('tags.html?name='+encodeURIComponent(cat[0]))}">${cat[0]}</a>`:''}
    ${title?` » ${title}`:''}
  `;
}
...
function renderTags(tags){
  $('#tagPills').innerHTML = (tags||[]).map(t=>`
    <a class="tag-pill" href="${withBase('tags.html?name='+encodeURIComponent(t))}">#${t}</a>
  `).join('');
}
...
(async function init(){
  ...
  // 读取 slug 的 md
  const slug = getSlug();
  const md = await fetchText('content/posts/' + slug + '.md');
  ...
  metaDesc && metaDesc.setAttribute('content', desc);
  ...
  // TOC
  window.tocbot && window.tocbot.init({
    tocSelector: '#toc',
